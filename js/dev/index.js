...
function initSliders() {
  ...
  if (document.querySelector(".says__slider")) {
...
function menuInit() {
  ...
  document.addEventListener("click", function(e) {
    if (bodyLockStatus && e.target.closest(".header__search")) {
      ...
      const input2 = document.querySelector(".header__input");
      input2.value = "";
      input2.focus();
    }
...
function digitsCounter() {
  const counters = document.querySelectorAll("[data-fls-digcounter]");
  if (!counters.length) return;
  const animateCounter = (el) => {
    const endValue = parseInt(el.textContent.replace(/\s+/g, ""), 10);
    const duration = +el.dataset.flsDigcounterSpeed || 1e3;
    let startTime = null;
    el.textContent = "0";
    const step = (time) => {
